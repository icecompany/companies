...
class CompaniesModelRegion extends AdminModel {

    public function getItem($pk = null)
    {
        $item = parent::getItem($pk);
        if ($item->id === null) {
            $item->companyID = JFactory::getApplication()->getUserState("{$this->option}.region.companyID");
        }
    ...
    public function getTable($name = 'Regions', $prefix = 'TableCompanies', $options = array())
    {
    ...
    protected function prepareTable($table)
    {
        ...
        unset($all['_errors']);
        $nulls = ['struct', 'comment']; //Поля, которые NULL
